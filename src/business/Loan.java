...
public class Loan extends Financial {
    ...
    public void run () {
        ...
        String title = "Thread" + Thread.currentThread().getId() + " Loan: " + curr.format(super.getAmt()) + ", " + 
                pct.format(super.getRate()) + super.getTerm() + " months has monthly payment of: " + curr.format(this.mopmt);
        if (this.tarea != null) {
            synchronized(tarea) {
            ...
            for (int i=0; i < tarea.getRowCount(); i++) {
               ...
               // break;
               // }
               String r = (String) tarea.getValueAt(i, 0);
               if (r == null) { r = ""; }
               if (r.isEmpty()) {
                   tarea.setValueAt(rate, i, 0);
                   tarea.setValueAt(title, i, 1);
                   break;
        ...
        String[] cols = { "Month", getBegBalDesc(), getPrinFactorDesc(), getPrinFactorDesc(), getEndBalDesc()};
        ...
        dg.add(sp);
        dg.setTitle(title);
        int vertoffset = (int) (new Random().nextInt(1000));
